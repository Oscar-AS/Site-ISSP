...
def index(request):
  ...
  liste_mot= Mots.objects.order_by('-Date')[:3]
...
def formulaire_message(request):
    if request.method == 'POST': 
      ...
      destinataire = 'projetsiteweblpas@gmail.com'  # Remplacez ceci par votre e-mail de réception
      sujet = f"Nouveau message de {nom} {prenom}"
      contenu = f"Nom: {nom}\nPrenom: {prenom}\nNumero: {numero}\nEmail: {email}\n\nMessage:\n{message}"
...
def connexion(request):
    if request.method == "POST":
        ...
        if user is not None:
            ...
            messages.error(request, "Votre nom d'utilisateur ou mot de passe est incorrect.")
            # Pour vider le formulaire, vous pouvez passer un dictionnaire vide comme contexte
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if User.objects.filter(username=username).exists():
                ...
                sujet = "Nouveau message de projetsiteweblpas@gmail.com"
                contenu = (f"Monsieur/Madame {nom} {prenom},\n\n"
                           f"Nous vous informons que votre compte d'utilisateur a été créé avec succès. "
                           f"Maintenant vous pouvez vous connecter sur le site en utilisant votre nom d'utilisateur et "
                           f"votre mot de passe.\n\nNom d'utilisateur : {username}\nMot de passe : {password}\n\n"
                           "Veuillez garder votre mot de passe secret.")

                send_mail(sujet, contenu, "projetsiteweblpas@gmail.com", [email])

                messages.success(request, "Votre compte a été créé avec succès. Un email de confirmation a été envoyé.")
